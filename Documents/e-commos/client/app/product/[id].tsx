...
export default function productDetails() {
  ...
  const fetchProduct = async () => {
    ...
    const found: any = dummyProducts.find((product) => product._id === id); setProduct(found ?? null);
    setLoading(false)
  }
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: 100 }}>
        ...
        <View className='relative h-[450px] bg-gray-100 mb-6'>
          <ScrollView horizontal pagingEnabled showsHorizontalScrollIndicator={false} scrollEventThrottle={16} onScroll={(e) => {
            ...
            {product.images?.map((img, index) => (
              <Image key={index} source={{ uri: img }} style={{ width: width, height: 400 }} resizeMode='cover' />
            ))}
          </ScrollView>
          {/* Header Actions */}
          <View className='absolute top-12 left-4 right-4 flex-row justify-between items-center z-10'>
            <TouchableOpacity onPress={() => router.back()} className='w-10 h-10 bg-white/80 rounded-full justify-center items-center'>
              <Ionicons name='arrow-back' size={24} color={COLORS.primary} />
            </TouchableOpacity>

            <TouchableOpacity onPress={() => toggleWishlist(product)} className='w-10 h-10 bg-white/80 rounded-full justify-center items-center'>
          ...
          <View className='absolute bottom-4 left-0 right-0 flex-row justify-center gap-2'>
            {product.images?.map((_, index) => (
              <View key={index} className={`h-2 rounded-full ${index === activeImage ? 'w-6 bg-primary' : 'w-2 bg-gray-300'}`} />
            ))}
          </View>
        ...
        <View className='px-5'>
         ...
         <View className='flex-row justify-between items-start mb-2'>
            <Text className='text-2x1 font-bold text-primary flex-1 mr-4'>{product.name}</Text>
            <View className='flex-row justify-between item-start mb-2'> 
         ...
         {product.sizes && product.sizes.length >0 && (
          ...
          <View className='flex-row gap-3 mb-6 flex-wrap'>
            {product.sizes.map((size)=>(
               <TouchableOpacity key={size} onPress={()=>setSelectedSize(size)} className={`w-12 h-12 rounded-full items-center justify-center border ${selectedSize ===size ? 'bg-primary boder-primary' : 'bg-white border-gray-100'}`}>
        ...
        {/* Footer */}
        <View className='absolute bottom-0 left-0 flex-row right-0 pg-4 bg-white border-t border-gray-100'>
        <TouchableOpacity onPress={handleAddtoCart} className='w-4/5 bg-primary py-4 rounded-full items-center shadow-lg flex-row justfy-center'>
